  ...
  async function handleSubmit(e) {
    ...
    if (userId) {
      const { error: profileError } = await supabase.from("profiles").insert([{
        id: userId, full_name: form.fullName, phone: form.phone, email: form.email, role: "owner",
      }]);
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input name="phone" type="tel" value={form.phone} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-900 focus:outline-none focus:border-[#1E88E5]" />
          </div>
          <div>
        ...
        <p className="text-gray-500 text-sm mt-6 text-center">
          ...
          <Link href="/login/owner" className="text-[#1E88E5] hover:underline">Log in</Link>
        </p>
      </div>
    </main>
  );
